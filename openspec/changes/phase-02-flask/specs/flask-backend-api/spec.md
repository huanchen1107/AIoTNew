...
The Flask application SHALL serve the dashboard `index_phase3.html` file (along with accessibility to the other phase-specific pages `index_phase1.html` and `index_phase2.html`) at the root URL `/`.

#### Scenario: User visits homepage
- **WHEN** a browser requests GET `/` from the Flask server
- **THEN** the server SHALL return `index_phase3.html` (or the corresponding phase-specific HTML file requested) with a HTTP 200 OK status
